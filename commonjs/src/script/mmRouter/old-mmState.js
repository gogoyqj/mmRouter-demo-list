    ...
    */
    ...
    var mmState = window.mmState = {
            ...
            isParamsChanged: function (p, op) {
                var isQuery = p == void 0, p = isQuery ? this.query : p, op = isQuery ? this.oldQuery : op, res = false;
                for (var i in p) {
                    if (!(i in op) || op[i] != p[i]) {
                        res = true;
                        break;
            ...
            popOne: function (end, args, callback) {
                ...
                cur.done = function (success) {
                    ...
                    if (success !== false) {
                        if (me.activeState)
                            return me.popOne(end, args, callback);
                    }
            ...
            pushState: function (end, args, callback) {
                ...
                while (end !== active && end) {
                    ...
                    // 拷贝父状态的视图文件
                    if (end)
                        tmp._local.template = avalon.mix({}, end._local.template, tmp._local.template);
                }
                // merge模板
                for (var i = chain.length - 1; i > -1; i--) {
                    ...
                    if (cv) {
                        for (var n in cv) {
                            var pts = n.split('@'), viewName = pts[0], statename = (pts.length == 2 ? pts[1] : cur.stateName) || '';
                            _local.template[viewName + '@' + (statename || (viewName ? '' : cur.stateName || ''))] = cv[n];
                            // 没有模板名字，却指定了其他的状态名，创建一个别名
                            if (!viewName && statename && statename != cur.stateName)
                                _local.template['@' + cur.stateName] = cv[n];
                        }
                    }
                }
                // 逐一迭代
            ...
            transitionTo: function (fromState, toState, args, options) {
                ...
                var info = avalon.router.urlFormate(toState.url, toState.params, mmState.query), me = this, options = options || {},
                    // 是否强制reload或者query发生变化，参照angular，这个时候会触发整个页面重刷
                    reload = options.reload || this.isParamsChanged(), over,
                    // 找共同的父节点，那么也是需要参考params和reload，reload情况，将所有栈里的state全部退出，否则退出到params没有发生变化的地方
                    commonParent = reload ? NaN : findCommonBranch(fromState, toState, args), done = function (success) {
                        if (over)
                            return;
                        over = true;
                        ...
                        if (info && avalon.history)
                            avalon.history.updateLocation(info.path + info.query, avalon.mix({}, options, { silent: true }));
                    };
                ...
                if (!reload && fromState == toState && !mmState.isParamsChanged(toState.oldParams, toState.params)) {
                    ...
                    if (toState == this.activeState && abort)
                        return done();
                    // 重复点击直接return
                    return;
                }
            ...
            avalon.each(state.views, function (keyname, view) {
                ...
                if (kn in currentState._local.template) {
                    ...
                    avalon.each(nodes, function (i, _node) {
                        var $node = avalon(_node), viewname = $node.attr('ms-view');
                        vn = viewname + '@' + (viewname ? '' : state.stateName || ''), vnCurrent = viewname + '@' + (mmState.currentState.stateName || '');
                        // 如何盘点这个节点是否需要回复默认内容
                        if (vn in currentState._local.template || vnCurrent in currentState._local.template || $node.hasClass('oni-mmRouter-leave') || avalon.contains(node, _node))
                            return;
                        // 通过这种方式方式消除重复刷新
                        var _html = $node.data(defKey), key = viewKey(_node);
                        if (_html) {
                    ...
                    if (node) {
                        var oldNode = avalon(node), key = viewKey(node);
    ...
    */
    ...
    StateModel.prototype = {
        paramsChanged: function () {
            ...
            if (res)
                this.oldParams = avalon.mix({}, this.params);
            return res;
        },
        _onChange: function () {
        ...
        */
        ...
        onBeforeChange: avalon.noop,
        // 切入某个state之前触发
        onChange: avalon.noop,
        // 切入触发
        onBeforeLoad: avalon.noop,
        // 所有资源开始加载前触发
        onAfterLoad: avalon.noop,
        // 加载后触发
        onBeforeUnload: avalon.noop,
        // state退出前触发
        onAfterUnload: avalon.noop
    };
    ...
    function findCommonBranch(fromState, toState, args) {
        var a = fromState && fromState.stateName || '', b = toState.stateName || '', partsA = a.split('.'), partsB = b.split('.'), i = 0, lenA = partsA.length;
        for (; i < lenA; i++) {
    ...
    function getStateByName(stateName) {
        var states = avalon.router.routingTable.get, state = NaN;
        for (var i = 0, el; el = states[i++];) {
    ...
    function getViews(ctrl, name, node) {
        ...
        var otherExpr = [], node = node || document;
        ...
        if (node.querySelectorAll) {
            ...
            seeds = matchSelectors(seeds, function (node) {
                return node.getAttribute('avalonctrl') === ctrl || node.getAttribute('ms-controller') === ctrl;
            });
    ...
    function viewKey(node) {
        var kn = 'view-id', key = node.getAttribute(kn);
        if (key)
            return key;
        key = Event.uiqKey();
    ...
    function fromPromise(config, params) {
        return config.template ? fromString(config.template, params) : config.templateUrl ? fromUrl(config.templateUrl, params) : config.templateProvider ? fromProvider(config.templateProvider, params) : new Promise(function (resolve, reject) {
